...
const Sidebar: React.FC<SidebarProps> = ({ profile, onLogout, collapsed, setCollapsed }) => {
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-2 py-4 space-y-1 overflow-auto">
        <AdminNavItem to="/admin" icon="dashboard" label="Dashboard" collapsed={collapsed} />
        <AdminNavItem
          to="/admin/posts"
          icon="posts"
        ...
        <AdminNavItem
          ...
          collapsed={collapsed}
        />
        <AdminNavItem to="/admin/analytics" icon="chart" label="Analytics" collapsed={collapsed} />
        <AdminNavItem to="/admin/settings" icon="settings" label="Settings" collapsed={collapsed} />
      </nav>

      {/* Logout */}
      <div className={`px-3 py-3 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
        <button
          onClick={onLogout}
          title="Log out"
          aria-label="Log out"
